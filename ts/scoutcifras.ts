(function ($) {
	...
	// plugin definition
	//
	$.fn.scoutcifras = ($$options) => {
		//;;; _debug(this);

		// build main options before element iteration
		let $settings = $.extend({}, $.fn.scoutcifras.defaults, $$options);
	...
	function createTAG(tag, opt) { let obj = document.createElement(tag); if (opt != null) { $(obj).attr(opt); } return obj; }
	//
	...
	function init($obj) {
		...
		$obj.Print.on('click', function () {
			let $newdiv = $obj.CodigoOut.clone();
